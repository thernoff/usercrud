            ...
            if (count($errors) == 0){                                
                if ($_FILES["image"]["name"]){
                    $config = \Application\Core\Config::instance()->data;
                        //Получаем пути к хранению исходный файлов ($path_main_image) и их уменьшенных копий
                        $path_main_image = $config['user']['path_main_image'];
                        $path_main_image_thumb = $config['user']['path_main_image_thumb'];
                        
                        $image = new \Application\Models\Image($_FILES["image"]["name"]);
                ...
                if (count($errors) == 0){
                    if ($_FILES["image"]["name"]){
                        ...
                        if ($image->createThumbnail($path_main_image, $path_main_image_thumb, $image->name, $width, $height)){
                            ...
                            if ($user->image && file_exists($path_main_image_thumb . $user->image)){
                                unlink($path_main_image_thumb . $user->image);
                            }
                            
                            if ($user->image && file_exists($path_main_image . $user->image)){
                                unlink($path_main_image . $user->image);
                            }
